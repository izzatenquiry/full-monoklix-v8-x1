...
export const fetchWithTokenRotation = async (
  ...
  if (isGenerationRequest) {
    if (onStatusUpdate) onStatusUpdate('Waiting for an available generation slot...');
    let slotAcquired = false;
    while (!slotAcquired) {
        const { data: acquired, error } = await supabase.rpc('request_generation_slot', { cooldown_seconds: 10 });
        if (error) {
    ...
    if (onStatusUpdate) onStatusUpdate('Slot acquired. Starting generation...');
  }
  // --- End Queueing Logic ---
